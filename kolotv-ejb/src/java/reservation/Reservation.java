...
public class Reservation extends ClassMere implements ClassIA {
    ...
    public List<reservation.ReservationDetails> decomposer(String nT, Connection c) throws Exception {
        ...
        if (listeFille == null)
            listeFille = (reservation.ReservationDetails[]) this.getFille(nT, c, "");
    ...
    public CheckOut[] getListeCheckOut(String nTableChekOut, Connection c) throws Exception {
        ...
        if (nTableChekOut != null && nTableChekOut.compareTo("") != 0)
            crt.setNomTable(nTableChekOut);
    ...
    public ReservationLibAvecDateMax getReservationWithMontant(Connection c) throws Exception {
        return (ReservationLibAvecDateMax) new reservation.ReservationLibAvecDateMax().getById(this.getId(),
                "reservation_lib_avecmaxdate", c);
    }
    ...
    public ReservationDetailsAvecDiffusion[] getReservationDetailsAvecDiffusions(Connection c) throws Exception {
        ...
        rd.setIdmere(this.getId());
        ReservationDetailsAvecDiffusion[] reservationDetails = (ReservationDetailsAvecDiffusion[]) CGenUtil
                .rechercher(rd, null, null, c, "");
        return reservationDetails;
    }
    ...
    public vente.VenteDetails[] genereVenteDetails(Connection c) throws Exception {
        ...
        try {
            ...
            String req = "SELECT IDPRODUIT,COUNT(QTE) AS QTES,PU FROM RESERVATIONDETAILS_LIB WHERE IDMERE = '"
                    + this.getId() + "' GROUP BY IDPRODUIT, PU";
    ...
    public static void imprimerExcel(XSSFWorkbook workbook, String idSupport, String idReservation, Date dt_debut,
            Date dt_fin, String etat, Connection c) throws Exception {
        ...
        for (Reservation reservation : list) {
            ...
            ReservationDetailsAvecDiffusion[] details = (ReservationDetailsAvecDiffusion[]) CGenUtil
                    .rechercher(resafille, null, null, c, "");
            Map<Integer, List<ReservationDetailsAvecDiffusion>> filles = ReservationDetailsAvecDiffusion
                    .groupByOrdre(details);
    ...
    public static void imprimerExcelJournalier(XSSFWorkbook workbook, String[] idTypeService, String idSupport,
            String dMin, String dMax, String etat, Connection c) throws Exception {
        Map<String, XSSFCellStyle> styleCache = new HashMap<>();
        int rowIndex = 0;
    ...
    public static List<ReservationDetailsAvecDiffusion> trierResa(List<ReservationDetailsAvecDiffusion> resaJ,
            Connection c) throws Exception {
        List<ReservationDetailsAvecDiffusion> result = new ArrayList<>();
        CategorieIngredient[] categorieIngredients = (CategorieIngredient[]) CGenUtil
                .rechercher(new CategorieIngredient(), null, null, c, " ORDER BY RANG ASC");
        ...
        for (Map.Entry<String, List<ReservationDetailsAvecDiffusion>> entry : listResaTrier.entrySet()) {
            ...
            for (CategorieIngredient cat : categoriesFiltrees) {
                List<ReservationDetailsAvecDiffusion> resaSimple = details.stream()
                        .filter(resa -> cat.getId().equals(resa.getCategorieproduit())
                                && resa.getIdparrainage() == null)
    ...
    public static List<ReservationDetailsAvecDiffusion> trierResaAvecBillboard(
            List<ReservationDetailsAvecDiffusion> resaJ, Connection c) throws Exception {
    ...
    public static void main(String[] args) throws Exception {
        ReservationDetailsAvecDiffusion[] list = (ReservationDetailsAvecDiffusion[]) CGenUtil
                .rechercher(new ReservationDetailsAvecDiffusion(), null, null, null, "");
    ...
    public List<Map<String, Object>> prepareDataModif(String dtDebut, String dtFin, Connection c) throws Exception {
        ...
        try {
            ...
            while (localDateDebut.isAfter(localDateFin) == false) {
                plannings.put(localDateDebut.format(DateTimeFormatter.ofPattern("dd/MM/yyyy")),
                        new String[] { "", "" });
            ...
            ReservationDetailsAvecDiffusion[] details = (ReservationDetailsAvecDiffusion[]) CGenUtil.rechercher(search,
                    null, null, c, " and DATY >= TO_DATE('" + dtDebut + "','YYYY-MM-DD') and DATY <= TO_DATE('" + dtFin
                            + "','YYYY-MM-DD')");
            Map<Integer, List<ReservationDetailsAvecDiffusion>> filles = ReservationDetailsAvecDiffusion
                    .groupByOrdre(details);
            ...
            for (Map.Entry<Integer, List<ReservationDetailsAvecDiffusion>> entry : filles.entrySet()) {
                ...
                resa.put("serviceLib",
                        resadet.get(0).getLibelleproduit() != null ? resadet.get(0).getLibelleproduit() : "");
    ...
    public Reservation updateResa(String u, String dtDebut, String dtFin, Connection c) throws Exception {
        ...
        try {
            ...
            String request = "DELETE FROM RESERVATIONDETAILS WHERE IDMERE=? AND ETAT!=? and DATY >= TO_DATE('" + dtDebut
                    + "','YYYY-MM-DD') and DATY <= TO_DATE('" + dtFin + "','YYYY-MM-DD')";
    ...
    public Reservation updateResaV2(String u, ReservationDetailsGroupe[] reservationDetailsGroupes, int lastOrder,
            Connection c) throws Exception {
        boolean estOuvert = false;
        try {
