<%--
    Document   : media-modif
    Created on : 21 mars 2024, 12:15:19
    Author     : Toky20
--%>

<%@page import="media.*"%>
<%@ page import="user.*"%>
<%@ page import="bean.*"%>
<%@ page import="utilitaire.*"%>
<%@ page import="affichage.*"%>
<%@ page import="produits.CategorieIngredient" %>

<%
    String autreparsley = "data-parsley-range='[8, 40]' required";
    Media t = new Media();

    String  mapping = "media.Media",
            nomtable = "Media",
            apres = "media/media-fiche.jsp",
            titre = "Modification media;";

    PageUpdate pu = new PageUpdate(t, request, (user.UserEJB) session.getValue("u"));
    pu.setLien((String) session.getValue("lien"));
    pu.setTitre("Modification media");
    pu.getFormu().getChamp("id").setAutre("readonly");

    pu.getFormu().getChamp("duree").setLibelle("Dur&eacute;e");
    pu.getFormu().getChamp("duree").setType("time");
    pu.getFormu().getChamp("duree").setAutre("step=\"1\"");
//    pu.getFormu().getChamp("duree").setDefaut(utilitaire.Utilitaire.heureCouranteHMS());

    pu.getFormu().getChamp("idTypeMedia").setLibelle("Type de M&eacute;dia");
    pu.getFormu().getChamp("idClient").setLibelle("Client");
    pu.getFormu().getChamp("idClient").setPageAppelComplete("client.Client","id","CLIENT");

    affichage.Champ[] liste = new affichage.Champ[1];
    CategorieIngredient typeMed= new CategorieIngredient();
    liste[0] = new Liste("idTypeMedia", typeMed, "val", "id");

    pu.getFormu().changerEnChamp(liste);

    String lien = (String) session.getValue("lien");
    String id=pu.getBase().getTuppleID();
    pu.preparerDataFormu();
%>
<div class="content-wrapper">
    <div class="row">
        <div class="col-md-3"></div>
        <div class="col-md-6">
            <div class="box-fiche">
                <div class="box">
                    <div class="box-title with-border">
                        <h1 class="box-title"><a href="<%= lien %>?but=media/media-fiche.jsp&id=<%=id%>"><i class="fa fa-arrow-circle-left"></i></a> <%=pu.getTitre()%></h1>
                    </div>
                    <form action="<%= lien %>?but=media/apresModifMedia.jsp&id=<%=id%>" method="post">
                        <%
                            out.println(pu.getFormu().getHtmlInsert());
                        %>
                        <div class="row">
                            <div class="col-md-11">
                                <button class="btn btn-primary pull-right" name="Submit2" type="submit">Valider</button>
                            </div>
                            <br><br>
                        </div>
                    </form>
                </div>
            </div>
        </div>
    </div>
</div>
